    ...
    if cfg.model.name in (
        ...
        if regression_net is None:
            ...
            loss_fn = DiffusionLoss(
                P_mean=cfg.model.get("P_mean", -1.2),
                P_std=cfg.model.get("P_std", 1.2),
                sigma_data=cfg.model.get("sigma_data", 0.5),
            )
            logger0.info("Using DiffusionLoss (diffusion-only / conditional-EDM, direct prediction)")
            assert not cfg.model.hr_mean_conditioning, \
                "diffusion-only 必须设 hr_mean_conditioning=False，否则条件通道数会和无回归均值的训练不一致"
        else:
