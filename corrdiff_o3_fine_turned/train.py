...
def main(cfg: DictConfig) -> None:
    ...
    # Load the model checkpoint: auto-resume > warm-start > random init
    warmstart_used = False
    checkpoints_exist = os.path.isdir(checkpoint_dir) and checkpoint_list(checkpoint_dir)
    if checkpoints_exist:
        # Path A: auto-resume from output directory
        cur_nimg_model = load_checkpoint(path=checkpoint_dir, models=model)
        logger0.success(f"Auto-resumed model from {checkpoint_dir}")
    elif (hasattr(cfg.training.io, "warmstart_checkpoint")
          and cfg.training.io.warmstart_checkpoint):
        # Path B: warm-start — load model weights only, no optimizer state
        warmstart_path = to_absolute_path(cfg.training.io.warmstart_checkpoint)
        if not os.path.exists(warmstart_path):
            raise FileNotFoundError(
                f"Warmstart checkpoint not found: {warmstart_path}"
            )
        warmstart_module = Module.from_checkpoint(warmstart_path, model_args)
        if dist.world_size > 1:
            model.module.load_state_dict(warmstart_module.state_dict())
        else:
            model.load_state_dict(warmstart_module.state_dict())
        del warmstart_module
        cur_nimg_model = 0
        warmstart_used = True
        logger0.success(
            f"Warm-started model weights from {warmstart_path} "
            f"(optimizer will be fresh, cur_nimg=0)"
        )
    else:
        # Path C: random init
        cur_nimg_model = None
        logger0.info("Randomly initializing model weights")

    # Freeze encoder (down blocks) if requested — must be warm-start, not auto-resume
    if hasattr(cfg.training.hp, 'freeze_encoder') and cfg.training.hp.freeze_encoder:
        if checkpoints_exist:
            raise RuntimeError(
                "freeze_encoder is incompatible with auto-resume. "
                "Use an empty output_dir for warm-start with frozen encoder."
            )
        if not warmstart_used:
            raise RuntimeError(
                "freeze_encoder requires warmstart_checkpoint to load weights first."
            )
        # Access inner model (skip DDP wrapper if present)
        inner = model.module if hasattr(model, 'module') else model
        frozen_count = 0
        trainable_count = 0
        for name, param in inner.named_parameters():
            if 'enc' in name.lower() or 'down' in name.lower():
                param.requires_grad = False
                frozen_count += param.numel()
            else:
                trainable_count += param.numel()
        logger0.info(
            f"freeze_encoder: {frozen_count:,} params frozen, "
    ...
    if cfg.model.name in (
        ...
        sigma_data = float(model_args.get("sigma_data", 0.5))
        loss_fn = ResidualLoss(
            regression_net=regression_net,
            hr_mean_conditioning=cfg.model.hr_mean_conditioning,
            sigma_data=sigma_data,
        )
        logger0.info(f"ResidualLoss sigma_data={sigma_data}")
    ...
    ## Load optimizer checkpoint if applicable
    if warmstart_used:
        cur_nimg = 0
        logger0.success("Optimizer freshly initialized, cur_nimg=0 (warm-start)")
    else:
        if dist.world_size > 1:
